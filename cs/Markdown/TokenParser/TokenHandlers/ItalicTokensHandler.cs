...
namespace Markdown.TokenParser.TagsGenerators;

public class ItalicTokensHandler : ITokenHandler
{
    public List<Token> HandleLine(List<Token> line)
    {
        ...
        for (var j = 0; j < line.Count; j++)
        {
            ...
            {
                var token = line[j];
                opened.Add(new Token(TokenType.MdTag, "_",
                    position, false, TagType.Italic));
            }
            else if (opened.Count > 0 && IsClosed(j, line))
            {
                result.Add(opened[0]);
                result.Add(new Token(TokenType.MdTag, "_",
                    position, true, TagType.Italic));

                opened.Clear();
            }
            else if (line[j].TagType == TagType.Italic)
            {
                result.Add(new Token(TokenType.Text, "_", position));
            }
    ...
    private bool IsOpenClosed(int index, List<Token> tokens)
    {
        return tokens.LastTokenIs(TokenType.Text, index) &&
               tokens.CurrentTokenIs(TagType.Italic, index) &
    ...
    {
        return index == 0 && tokens.CurrentTokenIs(TagType.Italic, index) &&
               tokens.NextTokenIs(TokenType.Text, index);
    }
    ...
    private bool IsLastInLine(int index, List<Token> tokens)
    {
        ...
        return isLast && tokens.CurrentTokenIs(TagType.Italic, index) &&
